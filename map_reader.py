...
def read_map(undo_manager: UndoManager,
    ...
    for y, row in enumerate(custom_map):
        for x, string in enumerate(row):
            if string == 'W':
                grid.add_piece(WallPiece(grid, undo_manager, animator, music_player, resources), Coordinate(x, y))
            if string == 'P':
                grid.add_piece(PlayerPiece(grid, undo_manager, animator, music_player, resources), Coordinate(x, y))
            if string == 'B':
                grid.add_piece(CratePiece(grid, undo_manager, animator, music_player, resources), Coordinate(x, y))
            if string == 'G':
                grid.add_piece(GoalPiece(grid, undo_manager, animator, music_player, resources), Coordinate(x, y))
